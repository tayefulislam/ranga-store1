...
const loadProducts = (keyword, id) => {
  ...
  if (keyword) {
    ...
    const datas = data.filter(element => element.title.toLowerCase().includes(`${keyword}`));
    // console.log(datas)
    showProducts(datas)
  ...
  if (id) {
    ...
    const single = data.find(element => element.id == id)
    // console.log(single);
...
<div class="modal-header">
            <h5 class="modal-title" id="detailsModalLabel">${details.title}</h5>
